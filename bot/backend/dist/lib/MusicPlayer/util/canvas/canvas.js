...
const TMP_DIR = process.env.IMG_TEMP_FOLDER;
const OUT_DIR = process.env.IMG_OUT_FOLDER;
logMessage("Image dirs: ", {
    TMP_DIR,
    OUT_DIR
});
...
const downloadThumbnail = async (youtubeId)=>{
    try {
        ...
        if (downloadThumbnail === null) throw new Error("Failed to download thumbnail");
...
const text = (instance, str, options)=>{
...
const borderRadius = (imagePath, cornerRadius)=>{
    return new Promise((resolve, reject)=>{
        gm(imagePath).size(async (err, size)=>{
            if (err) {
                ...
                const mask = await toTemp(gm(size.width, size.height, "none").fill("#ffffff").drawRectangle(0, 0, size.width, size.height, cornerRadius, cornerRadius));
...
const generatePlayingCard = (youtubeId, elapsedSeconds = 60)=>{
    ...
    return new Promise(async (resolve, reject)=>{
        var _this;
        var _lengthSeconds;
        const totalSeconds = (_lengthSeconds = (_this = await SongMetadataService.getOrCreateDisplayMetadata(youtubeId)) === null || _this === void 0 ? void 0 : _this.lengthSeconds) !== null && _lengthSeconds !== void 0 ? _lengthSeconds : 0;
        ...
        const blurredThumb = await toTemp(gm(thumbnail).modulate(70, 70).resize(w, h, "^") // Resize & crop to maintain aspect ratio
        .gravity("Center").extent(w, h) // Ensure the image is exactly 800x200
        .blur(50, 25));
        const smallH = h - padding * 2;
        const smallW = Math.round(smallH / 9 * 16);
        const roundedThumb = await borderRadius(thumbnail, 20);
        const smallThumb = await toTemp(gm(roundedThumb).resize(smallW, smallH, "^") // Resize & crop to maintain aspect ratio
        );
        const compositedThumbs = await toTemp(gm(blurredThumb).composite(smallThumb).geometry(`+${padding}+${padding}`));
        ...
        const playbarY1 = h - padding - playBarHeight - smFontSize - playbarTimestampPadding;
        const playbarX2 = rightColEnd;
        const playbarY2 = h - padding - smFontSize - playbarTimestampPadding;
        const withTitle = await toTemp(gm(compositedThumbs).font(SplineSans, fontSize).fill(c("gray", 50)).drawText(rightColStart, padding + fontSize, title).fontSize(smFontSize).drawText(rightColStart, Math.round(padding + fontSize * 2 + fontSize * 0.1), author).drawText(rightColEnd - 150, playbarY2 + smFontSize + playbarTimestampPadding, `${secondsToTimestamp(elapsedSeconds)}/${secondsToTimestamp(totalSeconds)}`));
        gm(withTitle).fill(c("gray", 300)).drawRectangle(playbarX1, playbarY1, playbarX2, playbarY2).fill(c("gray", 50)).drawRectangle(playbarX1, playbarY1, playbarX1 + elapsedNormalized * rightColW, playbarY2)// Add text

...
const data = new SlashCommandBuilder()
  .setName(commandName)
  .setDescription("Control settings regarding how BeanBot uses your data")
  .addSubcommand((subcommand) =>
    subcommand
      .setName(Subcommands.OptOut)
      .setDescription("Opt out of data collection")
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName(Subcommands.OptIn)
      .setDescription("Opt in to data collection")
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName(Subcommands.RequestData)
      .setDescription("Get a copy of your data in .json format")
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName(Subcommands.PrivacyPolicy)
      .setDescription("Get a link to the privacy policy")
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName(Subcommands.DeleteData)
      .setDescription("Delete all data BeanBot has about you (irreversible)")
  );
  ...
  switch (subcommand) {
    case Subcommands.OptIn:
      ...
      interaction.reply({
        ephemeral: true,
        content:
          "You've now opted in to your plays being used to recommend music, thank you! You can opt out at any time by running `/privacy optout`",
      });
      break;
    case Subcommands.OptOut:
      ...
      interaction.reply({
        ephemeral: true,
        content:
          "You've now opted out from all data collection, you can re-enable it at any time by running `/privacy optin`",
      });
      break;
    case Subcommands.RequestData:
      const userData = await UserDataService.requestData(userId);
      const attachment = new AttachmentBuilder(
        Buffer.from(JSON.stringify(userData, undefined, 2))
    ...
    case Subcommands.DeleteData:
      ...
      const filter = (interaction: any) =>
        interaction.customId === "delete_data" &&
        // @ts-ignore
    ...
    case Subcommands.PrivacyPolicy:
      ...
      break;
    default:
      interaction.reply({ content: "Unsupported command :/" });
      break;
  }
};
